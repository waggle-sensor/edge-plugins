                    ...
                    if len(data) >= self.HEADER_SIZE:
                        ...
                        if len(data) >= self.HEADER_SIZE + packet_length + self.FOOTER_SIZE:
                            packet = data[:self.HEADER_SIZE +
                                          packet_length + self.FOOTER_SIZE]
                            crc = packet[-2]
        ...
        try:
            ...
            with open(sensor_config_file, 'w') as config:
                config.write(json.dumps(
                    sensor_table, sort_keys=True, indent=4))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input-sensor', action='store_true',
                        help='Data from the sensor board')
    parser.add_argument('--hrf', action='store_true',
                        help='Print in human readable form')
